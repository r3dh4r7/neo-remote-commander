...
var App = {
    ...
    {
        try {
            var txs = await neon.api.neonDB.getTransactionHistory(Util.getConfig('network'), Util.getConfig('wallet'));
            ...
            if(tx.result.vout)
            {
                ...
                for(var i=0; i<tx.result.vout.length; i++) {
                    if(tx.result.vout[i].address != Util.getConfig('wallet')
    ...
    main: async function(verbosity)
    {
        try {
            ...
            if(lastChainTx && lastChainTx.asset == Util.getConfig('asset')
                && lastChainTx.address == Util.getConfig('wallet')
    ...
    init: function(delay, verbosity) {
        ...
        .then(function(successful) {
            if(successful)
            {
                ...
                console.error(chalk.red.bgBlack('Startup failed: could not fetch last blockchain state.'));
            }
        });
    }
}
